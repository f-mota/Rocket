...
if (isset($_GET['id'])) {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        header('Location: reservas.php?mensaje=La reserva ha sido CANCELADA correctamente.');
        exit();
    } else if ($stmt->affected_rows === 0 && $stmt->error == "") {
        // Si affected_rows es 0 y no hay error, puede que la reserva ya estuviera cancelada
        header('Location: reservas.php?mensaje=La reserva ya estaba cancelada o no se realizaron cambios.');
        exit();
    } else {
        // Mostrar un mensaje de error de la base de datos
        header('Location: reservas.php?mensaje=Error al CANCELAR la reserva: ' . $stmt->error);
        exit();
    }
} 
else {
    // Si no se pasó un ID, redirigir al listado de reservas
    header('Location: reservas.php?mensaje=No se reconoció ninguna reserva para cancelar.');
    exit();
}
